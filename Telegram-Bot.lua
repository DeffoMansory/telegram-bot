...
function main() 
    ...
    if thisScript().version ~= lastver then
        sampAddChatMessage(string.format('[Telegram-Bot]: {ffffff}Дуступно новое обновление: [%s > %s]', thisScript().version, lastver), 0xff6600)
        sendTelegramNotification(string.format('Бот успешно загружен.\nДоступно новое обновление: [%s > %s]\n/update для обновления скрипта', thisScript().version, lastver))
    else
